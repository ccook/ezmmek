##' Create standard curves (separate quench)
##'
...
g_std_curve <- function(d_std_g, x.label = NULL, y.label = NULL) {
  ...
  ### Add mean spec values of standard in buffer and homogenate to new dataframe
  d_std_g_2 <- d_std_g %>%
  ...
  g_std_curve_homo <- ggplot2::ggplot(data = d_std_g,
                                           mapping = ggplot2::aes(x = std.conc, y = spec.homo)) +
    ggplot2::geom_point() +
    ggplot2::theme_bw() +
  ...
  g_std_curve_buffer <- ggplot2::ggplot(data = d_std_g,
                                           mapping = ggplot2::aes(x = std.conc, y = spec.buffer)) +
    ggplot2::geom_point() +
    ggplot2::theme_bw() +
    ggplot2::xlab(x.label) +
    ggplot2::ylab(y.label) +
  ...
  g_std_curve_overlay <- ggplot2::ggplot() +
    ggplot2::geom_point(data = d_std_g_2, ggplot2::aes(x = std.conc, y = spec.buffer, color = "blue")) +
    ggplot2::geom_point(data = d_std_g_2, ggplot2::aes(x = std.conc, y = spec.homo, color = "red")) +
    ggplot2::theme_bw() +
    ggplot2::xlab(x.label) +
    ggplot2::ylab(y.label) +
    ggplot2::scale_y_continuous(labels = scales::scientific) +
    ggplot2::theme(axis.text = ggplot2::element_text(), axis.title = ggplot2::element_text()) +
    ggplot2::geom_smooth(data = d_std_g_2, ggplot2::aes(x = std.conc, y = spec.buffer), method = "lm", color = "blue") +
    ggplot2::geom_smooth(data = d_std_g_2, ggplot2::aes(x = std.conc, y = spec.homo), method = "lm", color = "red") +
